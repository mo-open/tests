...
class SupervisorActor extends CalculatorInt with PreStart with PostStop with Supervisor {
    ...
    def add(first: Int, second: Int): Future[Int] = Futures successful first + second

    // Non blocking request response
    def subtract(first: Int, second: Int): Future[Int] = Futures successful first - second
    ...
    def supervisorStrategy(): SupervisorStrategy =
        OneForOneStrategy(maxNrOfRetries = 10, withinTimeRange = Duration(5, SECONDS)) {
