package org.akka.essentials.unittest.example.java;
...
public class ExampleUnitTest extends TestKit {
	...
	public void testForwardingActor() {
		ActorRef forwardingActorRef = _system.actorOf(Props.create(
	...
	public void testSequencingActor() {
		...
		ActorRef sequencingActorRef = _system.actorOf(Props.create(
                new UntypedActorFactory() {
                    public UntypedActor create() {
	...
	public void testFilteringActor() {
		ActorRef filteringActorRef = _system.actorOf(Props.create(
                new UntypedActorFactory() {
                    public UntypedActor create() {
	...
	public void testSupervisorStrategy1() throws Exception {
		...
		ActorRef supervisorActorRef1 = _system.actorOf(Props.create(
                SupervisorActor.class), "supervisor1");

		Duration timeout = Duration.apply(5,"second");
		// register the BoomActor with the Supervisor
		final ActorRef child = (ActorRef) Await.result(
                ask(supervisorActorRef1, Props.create(BoomActor.class), 5000),
                timeout);
	...
	public void testSupervisorStrategy2() throws Exception {
		...
		ActorRef supervisorActorRef2 = _system.actorOf(Props.create(
				SupervisorActor.class), "supervisor2");
		
		final TestProbe probe = new TestProbe(_system);
		// register the BoomActor with the Supervisor
		final ActorRef child = (ActorRef) Await.result(
				ask(supervisorActorRef2, Props.create(BoomActor.class), 5000),
				Duration.apply(5,"second"));
		probe.watch(child);
		// second check
		child.tell("do something",ActorRef.noSender());
		probe.expectMsg(Terminated.apply(child,true,true));
	...
	public void testBoomActor() {
		final TestActorRef child = TestActorRef.apply(
				Props.create(BoomActor.class), _system);
		try {
