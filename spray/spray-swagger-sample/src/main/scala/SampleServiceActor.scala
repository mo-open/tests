...
import akka.actor._
import com.mlh.test.{CapService, PetHttpService}
...
class SampleServiceActor
    ...
    val caps = new CapService {
        def actorRefFactory = context
    }

    def receive = runRoute(pets.routes ~ caps.routes ~ users.routes ~ persons.routes ~ swaggerService.routes ~
        get {
            pathPrefix("") {
    ...
    val swaggerService = new SwaggerHttpService {
        override def apiTypes = Seq(typeOf[PetHttpService],
            typeOf[CapService], typeOf[UserHttpService], typeOf[PersonHttpService])
